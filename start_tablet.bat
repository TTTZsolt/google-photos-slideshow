...
where ssh >nul 2>nul
if %errorlevel% equ 0 (
    ssh -p 8022 u0_a116@192.168.1.157 "cd ~/swift-newton && nohup python -m backend.main > log.txt 2>&1 < /dev/null & exit"
) else (
    echo A rendszer SSH nem talalhato, probalom a Git mappabol...
    "C:\Program Files\Git\usr\bin\ssh.exe" -p 8022 u0_a116@192.168.1.157 "cd ~/swift-newton && nohup python -m backend.main > log.txt 2>&1 &"
)
